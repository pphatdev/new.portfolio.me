...
import { getAllPosts, getAllProjects } from '@/shared/libs/content';
import { AdminPageHeader } from '@/shared/components/admin/admin-page-header';
import { AdminTrafficSpotlight } from '@/shared/components/admin/admin-dashboard-chart';
    ...
    return (
        <div className="gap-6 grid grid-cols-1">
            ...
            <section className="grid gap-6 mt-1 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.95fr)]">
                        ...
                        <div className="flex flex-col gap-6">
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                <div className="max-w-2xl w-full grid gap-4">
                                    <Badge variant="outline" className="rounded-full border-primary/5 bg-background/80 px-3 py-1 text-[11px] uppercase tracking-[0.24em] text-muted-foreground">
                                        Live CMS Overview
                                    </Badge>
                ...
                <Card className="relative overflow-hidden rounded-4xl bg-background/80 py-0 dark:ring-2 ring-primary/50 backdrop-blur-sm">
                    <CardHeader className="border-b border-primary/5 px-6 pt-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {tagHighlights.length > 0 ? tagHighlights.map(([tag, count]) => (
                                    <Badge key={tag} variant="outline" className="rounded-full border-primary/5 bg-background/70 px-3 py-1 text-xs">
                                        {tag} · {count}
                                    </Badge>
                                )) : (
            ...
            <section className="grid gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(300px,0.9fr)]">
                <Card className="overflow-hidden rounded-[28px] bg-background/80 py-0 backdrop-blur-sm">
                    <CardHeader className="border-b border-primary/5 px-6 pb-5 pt-6">
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 pt-1">
                                ...
                                <Link href="/admin/posts" className="text-xs text-muted-foreground transition-colors hover:text-foreground">
                                    View all →
                                </Link>
                            </div>
                        </div>
                    ...
                    <CardContent className="space-y-3 px-6 py-6 pt-0">
                        {recentActivity.map((item) => (
                            <Link
                                ...
                                href={item.href}
                                className="flex items-start gap-4 rounded-2xl border border-primary/5 bg-foreground/20 px-4 py-3 transition-colors hover:border-primary/5 hover:bg-muted/35"
                            >
                                <div className="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-2xl border border-primary/5 bg-background/90 text-foreground">
            ...
            <section className="grid gap-6 2xl:grid-cols-3">
                <Card className="overflow-hidden rounded-[28px] border-primary/5 bg-background/80 py-0 backdrop-blur-sm xl:col-span-1">
                    <CardHeader className="border-b border-primary/5 px-6 pt-6">
                    ...
                    <CardContent className="space-y-3 px-6 py-6 pt-0">
                        {draftQueue.length > 0 ? draftQueue.map((item) => (
                            <Link
                                ...
                                href={item.href}
                                className="flex items-center gap-3 rounded-2xl border border-primary/5 bg-foreground/20 px-4 py-3 transition-colors hover:border-primary/5 hover:bg-muted/35"
                            >
                            ...
                            <div className="rounded-3xl border border-dashed border-primary/5 px-4 py-8 text-center text-sm text-muted-foreground">
                    ...
                    <CardContent className="space-y-6 px-6 py-6 pt-0">
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                ...
                                <Link href="/admin/posts" className="text-sm text-muted-foreground transition-colors hover:text-foreground">View all</Link>
                            </div>
                            {recentPosts.map((post) => (
                                <Link key={post.id} href={`/admin/posts/${post.id}`} className="flex items-center justify-between gap-3 rounded-2xl border border-primary/5 bg-foreground/20 px-4 py-3 transition-colors hover:border-primary/5 hover:bg-muted/35">
                        ...
                        <div className="space-y-3">
                            ...
                            {recentProjects.map((project) => (
                                <Link key={project.id} href={`/admin/projects/${project.id}`} className="flex items-center justify-between gap-3 rounded-2xl border border-primary/5 bg-foreground/20 px-4 py-3 transition-colors hover:border-primary/5 hover:bg-muted/35">
                                    <div className="min-w-0">
                ...
                <Card className="overflow-hidden rounded-[28px] border-primary/5 bg-background/80 py-0 backdrop-blur-sm xl:col-span-1">
                    ...
                    <CardContent className="space-y-6 px-6 py-6 pt-0">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                            <Link href="/admin/posts" className="rounded-2xl border border-primary/5 bg-foreground/5 p-4 transition-colors hover:border-primary/5 hover:bg-muted/30">
                                <div className="flex items-center justify-between gap-3">
                            ...
                            <Link href="/admin/projects" className="rounded-2xl border border-primary/5 bg-foreground/5 p-4 transition-colors hover:border-primary/5 hover:bg-muted/30">
                                <div className="flex items-center justify-between gap-3">

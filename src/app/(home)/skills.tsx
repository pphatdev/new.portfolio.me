...
export const SectionSkills = () => {
    ...
    return (
        <section id="skills" className="max-w-5xl flex pt-40 flex-col items-center justify-center mx-auto">
            <BlurFade delay={0.25} inView className="flex flex-col items-start justify-center pb-1 mt-5">
                <Badge variant="outline" className='py-1.5 px-3 lg:ml-5'>Skills</Badge>
                <Title as='h2' title={title} description={description} />
                <div className="w-full flex gap-3 mb-5 rounded-2xl shadow-card shadow-primary/5 bg-background/10 p-8 px-5 flex-wrap justify-start">
                    {languages.map((lang, key) => (
                        <MagneticArea key={key}>
                            <div className={cn(
                                "relative flex max-sm:h-9 max-sm:w-9 shadow-card size-12 p-0.5 rounded-2xl items-center justify-center overflow-hidden border transition-all duration-300 hover:scale-110",
                                colors[key % colors.length]
                            )}>
                                {lang.svg ? (
                                    <div className="w-full h-full z-10 [&>svg]:w-full [&>svg]:h-full [&>svg]:object-contain [&>svg]:text-current [&>svg>[data-foreground]]:text-white" dangerouslySetInnerHTML={{ __html: lang.svg }} />
                                ) : (
                                    <Image src={lang.src} alt={lang.alt} width={40} height={40} className="object-contain p-2 z-10" />
                                )}
                            </div>
                        </MagneticArea>
                    ))}

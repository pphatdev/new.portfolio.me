...
export function AdminTrafficSpotlight({ items }: { items: TrafficItem[] }) {
    if (items.length === 0) {
        return (
            <div className="flex min-h-48 items-center justify-center rounded-3xl border border-dashed border-primary/5 bg-background/50 text-sm text-muted-foreground">
            ...
            {items.map((item, index) => {
                ...
                return (
                    <Link
                        key={item.href}
                        href={item.href}
                        className="group flex items-center gap-4 rounded-2xl border border-primary/5 bg-muted/20 px-4 py-3.5 transition-all hover:border-primary/5 hover:bg-muted/40 hover:shadow-sm"
                    >
                        {/* Rank */}
                        <div className="flex size-8 shrink-0 items-center justify-center rounded-xl border border-primary/5 bg-background/80 text-xs font-semibold tabular-nums text-muted-foreground transition-colors group-hover:border-primary/5 group-hover:text-foreground">
                            {index + 1}
                        </div>
                        ...
                        <div className="min-w-0 flex-1 space-y-2">
                            <div className="flex min-w-0 items-center gap-2">
                                ...
                                {isDraft && (
                                    <span className="shrink-0 rounded-full border border-primary/5 px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.18em] text-muted-foreground">
                                        Draft
                                    </span>
                                )}
                            </div>

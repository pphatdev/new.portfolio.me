...
const ProjectCard: React.FC<{ project: Project, className?: string }> = ({ project, className }) => {
    ...
    return (
        <div
            className={cn("col-span-1 top-5 sticky duration-300 drop-shadow-xl drop-shadow-black/2 overflow-hidden max-sm:p-1 p-0.5 max-sm:rounded-none bg-background group font-sans rounded-4xl mb-4 ring-1 ring-foreground/5 hover:ring-primary/50 hover:ring-2 transition-all ease-in-out flex flex-col h-full", className)}
            role="article"
            tabIndex={-1}>

            <Image src={project.thumbnail} width={512} height={512} alt={project.title || ""} className="object-cover w-full aspect-video duration-300 transition-all ease-in-out max-sm:rounded-none rounded-4xl" />
            ...
            <h2 className="text-lg px-4 mt-5 z-10 font-semibold font-sans leading-snug tracking-tight line-clamp-2 pb-1">{project.title}</h2>
            <header className='mb-2 px-4 relative flex mt-2 justify-between items-center'>
            ...
            <p className='font-normal px-4 z-10 line-clamp-2 text-foreground/80'>
                {project.description}
            </p>
